...
def create_target(df_raw, window=60):
    ...
    미래 60일간의 평균 가격을 계산하고, 현재가 대비 기대 수익률(%)을 타겟으로 설정합니다.
    """
    print(f"타겟 변수 생성(미래 {window}일 평균 수익률)을 시작합니다...")
    
    # 1. 미래 60일간의 평균 가격 계산 (shift를 통해 미래 데이터를 현재 시점으로 가져옴)
    # rolling().mean()은 과거 데이터를 평균내므로, -window 만큼 shift하여 미래 데이터를 평균냄
    future_avg = df_raw['Gold'].shift(-window).rolling(window=window).mean()
    
    # 2. 현재가 대비 기대 수익률(%) 계산
    # 타겟 = (미래평균 / 현재가 - 1) * 100
    target = (future_avg / df_raw['Gold'] - 1) * 100
